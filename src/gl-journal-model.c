...
static void
gl_journal_model_process_query (GlJournalModel *model, gboolean is_search_field_exact)
{
                             ...
                             GlQuery *query)
{
    ...
    gl_journal_model_process_query (model, query->is_search_field_exact);
}
                                   ...
                                   gboolean case_sensitive)
{
    ...
    if (case_sensitive)
    {
        if (strstr ("_MESSAGE", search_match->field_name)) return message;

        else if (strstr ("_COMM", search_match->field_name)) return comm;

        else if (strstr ("_KERNEL_DEVICE", search_match->field_name)) return kernel_device;

        else if (strstr ("_AUDIT_SESSION", search_match->field_name)) return audit_session;

        else if (strstr ("_SYSTEMD_UNIT", search_match->field_name)) return systemd_unit;

        else if (strstr ("_PID", search_match->field_name)) return pid;

        else if (strstr ("_UID", search_match->field_name)) return uid;

        else if (strstr ("_GID", search_match->field_name)) return gid;

        else if (strstr ("_EXE", search_match->field_name)) return executable_path;
    }
    else
    {
        if (utf8_strcasestr ("_message", search_match->field_name)) return message;

        else if (utf8_strcasestr ("_comm", search_match->field_name)) return comm;

        else if (utf8_strcasestr ("_kernel_device", search_match->field_name)) return kernel_device;

        else if (utf8_strcasestr ("_audit_session", search_match->field_name)) return audit_session;

        else if (utf8_strcasestr ("_systemd_unit", search_match->field_name)) return systemd_unit;

        else if (utf8_strcasestr ("_pid", search_match->field_name)) return pid;

        else if (utf8_strcasestr ("_uid", search_match->field_name)) return uid;

        else if (utf8_strcasestr ("_gid", search_match->field_name)) return gid;

        else if (utf8_strcasestr ("_exe", search_match->field_name)) return executable_path;
    }
